...
void Setup(void) {
  CCP = 0xD8;             // Unlock clock prescaler register
  CLKPSR = 0b0000;        // Set clock to 8 MHz
  
  TCCR0A = (3<<COM0B0) | (2<<WGM00); // Datasheet p100; set PB1 (Pin 3) as PWM pin
  TCCR0B = (3<<WGM02) | (2<<CS00);   // Datasheet p103; set PWM behavior, set clock scaler to 1 MHz
  OCR0B = 1023;                      // Counter for PWM on PB1 (pin 3)
  ICR0 = 1023;                       // Counter resets approx. every millisecond, good for timing
  OCR0A = 511;                       // A nice middle-of-the-range value for timing during PWM
...
void sendSignal(char signal) {
  // If signal is a logical 1: clear PORTB0 for 20 μs, set PORTB0 for 80 μs, exit
  // If signal is a logical 0: clear PORTB0 for 80 μs, set PORTB0 for 20 μs, exit
  if (signal) {
    PORTB &= 0b1110;
    _delay_us(20);
    PORTB |= 0b0001;
